...
class ControlMap(OverlayBase):
    ...
    def compute(self, game_state):
        ...
        supply_control = {HL: set(), WS: set()}
        if str(getattr(game_state, "supply", "standard")).strip().lower() == "advanced":
            for side in (HL, WS):
                supply_control[side] = _compute_supply_reach(game_state, side)
        ...
        for row in range(board.height):
            for col in range(board.width):
                ...
                sides = set()
                for side in (HL, WS):
                    if key in zoc_by_side[side] or key in supply_control[side]:
                        sides.add(side)
                if len(sides) == 1:
...
class EnemyPowerMap(OverlayBase):
    name = "enemy_power"
    ...
    def compute(self, game_state):
        ...
        if enemy is None:
            return OverlayData(kind="scalar", values={})
        influence = InfluenceMap(enemy).compute(game_state)
        influence.meta["side"] = enemy
        return influence


class OddsMap(OverlayBase):
    name = "odds"

    def compute(self, game_state):
        active = getattr(game_state, "active_player", None)
        if active not in (HL, WS):
            return OverlayData(kind="scalar", values={})
        friendly = InfluenceMap(active).compute(game_state)
        enemy = InfluenceMap(_enemy_of(active)).compute(game_state)
        values = {}
        max_val = 0.0
        for key, f_power in friendly.values.items():
            e_power = enemy.values.get(key, 0.0)
            ratio = _safe_ratio(f_power, e_power)
            values[key] = ratio
            max_val = max(max_val, abs(ratio))
        return OverlayData(kind="scalar", values=values, min_value=-max_val, max_value=max_val)
...
def _safe_ratio(attacker: float, defender: float) -> float:
    if defender <= 0:
        return 6.0
    if attacker <= 0:
        return 0.0
